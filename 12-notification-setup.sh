...
ACTION_EOF
    ...
    # Update jail.local to use ntfy notifications
    if grep -q "action = " /etc/fail2ban/jail.local; then
        log_info "Updating jail.local with ntfy action..."

        # Backup current jail.local
        cp /etc/fail2ban/jail.local "/etc/fail2ban/jail.local.backup.$(date +%s)"

        # This is optional - users may want to add ntfy to their existing actions
        log_info "To enable ntfy notifications, add this to [DEFAULT] section in jail.local:"
        log_info "action = %(action_)s"
        log_info "         proxmox-ntfy"

    else
        log_info "jail.local already has custom actions"
    fi
